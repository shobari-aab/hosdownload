...
class RecorderEventHandler : public agora::rtc::IAgoraMediaRtcRecorderEventHandler{
  public:
    ...
    void printActiveUsers() {
        std::lock_guard<std::mutex> lock(mtx);
        if (uids_.empty()) {
            AG_LOG(INFO, "STUCK_DEBUG: No users in set.");
            return;
        }
        for (const auto& id : uids_) {
            AG_LOG(INFO, "STUCK_DEBUG: User still considered ACTIVE: %s", id.c_str());
        }
    }
  ...
  private:
    ...
    bool isSubscribeStream(agora::user_id_t uid, bool isAudio);
