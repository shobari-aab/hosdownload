...
int main(int argc, char * const argv[]) 
{
  ...
  AG_LOG(INFO, "isMix=%d, about to stop", config.isMix);
  if(config.isMix){
    AG_LOG(INFO, "Stopping recording with stopRecording()...");
    recorder->stopRecording();
  } else {
    AG_LOG(INFO, "Stopping individual user recording with stopSingleRecordingByUid()...");
